...
if(FALSE) {
  ...
  phases %>%  group_by(phase) %>% summarise(min.x = min(x)) -> phase.x
  phases %>%  group_by(phase) %>% summarise(mean = mean(x)) -> phase.annotate
  ...
  datCrI %>% filter(Type %in% "canton") %>% mutate(title = "By canton") %>% 
    ...
    geom_errorbar(aes(x = x, ymin = LL, ymax = UL), width = 0) + 
    geom_point(aes(x = x, y = median), size = 1) + 
    scale_x_continuous(breaks = 1:26, labels = datCrI$categories[-c(1:13)]) + 
    ylim(c(-20, 40)) + 
    theme_bw() + xlab("") + ylab("") + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    geom_hline(yintercept = 0, col = "red", linetype = "dotted") + 
  ...
  png(paste0(controls$savepoint,"/Fig1_up.png"), width = 16, height = 16, res = 300, units = "cm")
