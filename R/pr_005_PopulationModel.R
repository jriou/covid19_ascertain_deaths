...
# Model for population Switzerland
...
pop <- read.csv("data/popCH2010_2020.csv", sep = ";", header = TRUE)

head(pop)
colnames(pop)[2] <- "canton"
pop$Bevölkerungstyp <- NULL
...
pop %>% rowwise() %>% mutate(X0_39 = sum(c_across(X0.Jahre:X39.Jahre), na.rm = T), 
  ...
  select(Jahr, canton, Geschlecht, X0_39, X40_59, X60_69, X70_79, X80plus) -> pop
...
# plot(pois.samples[,15], pois.samples[,190])
# abline(a = 0, b = 1) # looks fine

datsamples <- data[, c("year", "canton", "sex", "age", "population")]
datsamples <- cbind(datsamples, pois.samples)

# plot(datsamples$population, pois.samples[,100])
# abline(a = 0, b = 1)
# 
# plot(
# datsamples$population[!is.na(datsamples$population)], 
# pois.samples[!is.na(datsamples$population),20]
# )
...
##
## random slope by age*sex*canton
# formula = 
#   population ~ 1 +  
#   year + factor(sex) +
#   f(id.time, model='iid', hyper=hyper.iid, constr = TRUE) + 
...
# 
# m <- inla.rerun(m)
# post.samples <- inla.posterior.sample(n = 1000, result = m)
# poppred <- lapply(post.samples, function(X) exp(X$latent[is.na(data$population)]))
# 
# pred.samples <- do.call(cbind, poppred)
# set.seed(11)
# pois.samples <-  apply(pred.samples, 2, function(X) rpois(n = length(X), lambda = X))
# 
# 
# datsamples <- data[is.na(data$population), c("year", "canton", "sex", "age")]
# datsamples <- cbind(datsamples, pois.samples)
# 
# saveRDS(datsamples, file = "pois.samples.populaiton.VC")
